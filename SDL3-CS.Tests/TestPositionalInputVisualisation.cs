...
namespace SDL.Tests
{
    public unsafe class TestPositionalInputVisualisation : RunAppTest
    {
        ...
        {
            new TestPositionalInputVisualisation().TestEnterMainCallbacks();
        }
        ...
        private readonly SortedDictionary<(SDL_TouchID TouchID, SDL_FingerID FingerID), PointF> activeTouches = new();
        private readonly SortedDictionary<SDL_MouseID, PointF> activeMice = new();
        private readonly SortedDictionary<SDL_PenID, PointF> activePens = new();

        private static (byte r, byte g, byte b) getColor(object o)
        {
            int color = o.ToString()?.GetHashCode() ?? 0;
            byte r = (byte)color;
            byte g = (byte)(color / 256);
            byte b = (byte)(color / 256 / 256);
            return (r, g, b);
        }
