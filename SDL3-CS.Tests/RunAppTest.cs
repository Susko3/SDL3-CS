...
namespace SDL.Tests
{
    ...
    public abstract unsafe class RunAppTest
    {
        [Test]
        public void TestEnterMainCallbacks()
        {
            var objectHandle = new ObjectHandle<RunAppTest>(this, GCHandleType.Normal);
        ...
        private static int AppInit(IntPtr* appState, int argc, byte** argv)
        {
            ...
            *appState = handle;
            ...
            var objectHandle = new ObjectHandle<RunAppTest>(handle, true);
        ...
        {
            var objectHandle = new ObjectHandle<RunAppTest>(appState, true);
            if (objectHandle.GetTarget(out var target))
        ...
        {
            using var objectHandle = new ObjectHandle<RunAppTest>(appState, true);
            if (objectHandle.GetTarget(out var target))
                target.Quit();
        }
    }
